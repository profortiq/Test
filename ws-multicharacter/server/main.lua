...
local function GiveStarterItems(source)
    ...
    for _, v in pairs(QBCore.Shared.StarterItems) do
        ...
        exports['qb-inventory']:AddItem(src, v.item, v.amount, false, info, 'ws-multicharacter:GiveStarterItems')
    end
end
...
QBCore.Commands.Add('logout', Lang:t('commands.logout_description'), {}, false, function(source)
    ...
    TriggerClientEvent('ws-multicharacter:client:chooseChar', src)
end, 'admin')

QBCore.Commands.Add('closeNUI', Lang:t('commands.closeNUI_description'), {}, false, function(source)
    local src = source
    TriggerClientEvent('ws-multicharacter:client:closeNUI', src)
end)
...
RegisterNetEvent('ws-multicharacter:server:disconnect', function()
    local src = source
    DropPlayer(src, Lang:t('commands.droppedplayer'))
end)

RegisterNetEvent('ws-multicharacter:server:loadUserData', function(cData)
    local src = source
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        if Config.SkipSelection then
            ...
            TriggerClientEvent('ws-multicharacter:client:spawnLastLocation', src, coords, cData)
        else
...
RegisterNetEvent('ws-multicharacter:server:createCharacter', function(data)
    local src = source
    local newData = {}
    ...
    if QBCore.Player.Login(src, false, newData) then
        ...
        if GetResourceState('qb-apartments') == 'started' and Apartments.Starting then
            ...
            QBCore.Commands.Refresh(src)
            loadHouseData(src)
            TriggerClientEvent('ws-multicharacter:client:closeNUIdefault', src)
            GiveStarterItems(src)
...
RegisterNetEvent('ws-multicharacter:server:deleteCharacter', function(citizenid)
    local src = source
...
QBCore.Functions.CreateCallback('ws-multicharacter:server:GetUserCharacters', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('ws-multicharacter:server:GetServerLogs', function(_, cb)
    MySQL.query('SELECT * FROM server_logs', {}, function(result)
        cb(result)
    end)
end)

QBCore.Functions.CreateCallback('ws-multicharacter:server:GetNumberOfCharacters', function(source, cb)
    local src = source
    local license = QBCore.Functions.GetIdentifier(src, 'license')
    local numOfChars = 0
...
QBCore.Functions.CreateCallback('ws-multicharacter:server:setupCharacters', function(source, cb)
...
QBCore.Functions.CreateCallback('ws-multicharacter:server:getSkin', function(_, cb, cid)

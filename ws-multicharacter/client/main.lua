...
CreateThread(function()
    while true do
        ...
        if NetworkIsSessionStarted() then
            TriggerEvent('ws-multicharacter:client:chooseChar')
            return
        end
    end
end)
...
local function openCharMenu(bool)
    QBCore.Functions.TriggerCallback('ws-multicharacter:server:GetNumberOfCharacters', function(result, countries)
        local translations = {}
        ...
        SendNUIMessage({
            ...
            premiumSlotIndex = Config.PremiumSlotIndex,
            premiumSlotIsFree = Config.PremiumSlotIsFree,
            portraits = Config.SlotPortraits or {},
        })
        skyCam(bool)
...
RegisterNetEvent('ws-multicharacter:client:closeNUIdefault', function() -- This event is only for no starting apartments
...
RegisterNetEvent('ws-multicharacter:client:closeNUI', function()
    DeleteEntity(charPed)
    SetNuiFocus(false, false)
    SendNUIMessage({
        action = 'forceClose',
...
RegisterNetEvent('ws-multicharacter:client:chooseChar', function()
    SetNuiFocus(false, false)
    DoScreenFadeOut(10)
    Wait(1000)
...
RegisterNetEvent('ws-multicharacter:client:spawnLastLocation', function(coords, cData)
...
RegisterNUICallback('closeUI', function(_, cb)
    ...
    DoScreenFadeOut(10)
    TriggerServerEvent('ws-multicharacter:server:loadUserData', cData)
    openCharMenu(false)
...
RegisterNUICallback('disconnectButton', function(_, cb)
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
    TriggerServerEvent('ws-multicharacter:server:disconnect')
    cb('ok')
end)
...
RegisterNUICallback('cDataPed', function(nData, cb)
    ...
    if cData ~= nil then
        if not cached_player_skins[cData.citizenid] then
            ...
            QBCore.Functions.TriggerCallback('ws-multicharacter:server:getSkin', function(model, data)
...
RegisterNUICallback('setupCharacters', function(_, cb)
    QBCore.Functions.TriggerCallback('ws-multicharacter:server:setupCharacters', function(result)
        cached_player_skins = {}
        SendNUIMessage({
...
RegisterNUICallback('createNewCharacter', function(data, cb)
    ...
    TriggerServerEvent('ws-multicharacter:server:createCharacter', cData)
    SetNuiFocus(false, false)
    cb('ok')
end)

RegisterNUICallback('removeCharacter', function(data, cb)
    TriggerServerEvent('ws-multicharacter:server:deleteCharacter', data.citizenid)
    DeletePed(charPed)
    TriggerEvent('ws-multicharacter:client:chooseChar')
    cb('ok')
end)
